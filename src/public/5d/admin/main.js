...
function messNewJoin2(datas) {
    ...
    datas.map((data) => {
        ...
        <div class="direct-chat-text" style="background-color: #1eb93d">User: ${
            data.phone
        } -> Tham gia <input type="text" style="background-color: #1eb93d; width:55px; border: 1px solid white" value="${
            data.join_bet
        }" id="${
            data.id
        }-join_bet" /> <input type="text" style="background-color: #1eb93d; width:55px; border: 1px solid white" value="${
            isNumber(list_join2)
                ? list_join2
                : list_join2 == 'b'
                ? 'Lớn'
