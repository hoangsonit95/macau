...
function cz_show(slug, e) {
  e.parent().find('.active').removeClass('active');
  e.addClass('active');
  e.parent().parent().find('.table-body').hide();
  e.parent()
    .parent()
    .find('.cz_' + slug)
    .show();
...
$(document).on(
  'click',
  '#k3hzzx .ball_number,#k3sthtx .ball_number,#k3slhtx .ball_number',
  function () {
    var obj = $(this);

    var text = obj.text(),
      number = obj.attr('ball-number'),
      playid = obj.attr('playid');
    ...
    if (playid == 'k3slhtx') {
      ...
      $('#k3hzzx .ball_number.curr').each(function (index) {
        const peilv = $(this).attr('peilv');
        console.log(peilv, 'peilv');
        var numbers = $(this).attr('ball-number');
        if (numbers == '大') {
          var numberss = 'Lớn';
        } else if (numbers == '小') {
        ...
        if (parseInt(ratea.minjj) < 1 || !ratea.minjj) ratea.minjj = 1;
        ...
        if (lotteryRate == 404) {
          ...
          array['rate'] = (
            dufrate - (lotteryFandian * fandn).toFixed(2)
          ).toFixed(2);
        } else {
          array['rate'] =
            lotteryRate == 0 || lotteryRate === 'undefined'
              ? ratea.rate
              : lotteryRate;
        }
        console.log(array);
...
$(document).on('click', '#k3sthdx .ball_item', function () {
  var obj = $(this);
  var text = obj.text(),
    number = obj.find('a').attr('ball-number'),
    playid = 'k3sthdx';
  var rate = rates[playid];
  var bools = true;
...
$(document).on('click', '#k3ethdx .ball_item', function () {
  ...
  $('#k3ethdx .ball_item')
    .find('a[ball-number="' + removeNumber + '"]')
    .removeClass('curr');
  $('#lottery_sum_old_b')
    .find('em[lottery_sum="' + removeNumber + '"]')
    .remove();

  var numberarray = [];
  $('#k3ethdx li ul')
    .eq(0)
    .find('.ball_number.curr')
    .each(function (index) {
      var number = $(this).attr('ball-number');
      numberarray.push(number);
    });
...
function submit_order(_playid, _number, number, zhushu, obj, ishz) {
  if (!ishz) {
    ...
    var array = {
      ...
      maxzs: rate.maxzs,
      rate:
        lotteryRate == 0 || lotteryRate === undefined ? rate.rate : lotteryRate,
      fandian: lotteryFandian,
    };
    orderList = new Array();
...
$(document).on('click', '.betting_right_btn-submit', function () {
  // console.log(orderList,lottery.currFullExpect,lotteryname);

  if (
    $('.lottery_input').val() == '' ||
    parseInt($('.lottery_input').val()) < 1
  ) {
    layer.open({
      time: 2,
      content: 'Vui lòng đặt số tiền đặt cược',
    });
    // alt("Vui lòng đặt số tiền đặt cược", -1);
    return false;
  }

  if (
    parseInt($('.betting_sum').text()) < 1 ||
    parseInt($('.betting_sum').val()) < 1
  ) {
    layer.open({
      time: 2,
      content: 'Chọn ít nhất một lần đặt cược',
    });
    // alt("Chọn ít nhất một lần đặt cược", -1);
    return false;
  }
  var lottery_number = '';
  var lottery_moneyOld = parseInt($('.betting_sum_moery').text());
  console.log(lottery_moneyOld, 'lottery_moneyOld');
  ...
  $('#lottery_sum_old_b')
    .find('.lottery_sum_old')
    .each(function () {
      var mnumbers = $(this).attr('lottery_sum');
      if (mnumbers == '大') {
        var mmnumbers = 'Lớn';
      } else if (mnumbers == '小') {
        var mmnumbers = 'Nhỏ';
      ...
      lottery_number = lottery_number + mmnumbers + '/'; //+ $(this).attr('lottery_sum') + '/';
    });
  var html = '';
  console.log('tfewfềwf');
  html +=
    '<div class="clearfix"><div class="pull-left" style="width:100%; text-align:initial">' +
    lottery?.shortname +
    ':</div><div class="pull-left" way-data="showExpect.shortname">' +
    lottery?.currFullExpect +
    'Trông chờ</div></div>';
  html +=
    '<div class="clearfix"><div class="pull-left" style="width:100px;">Cá cược:</div><div style="margin-left:0; text-align: left;white-space:normal;word-break:break-all;word-wrap:break-word:;"><span style="color:red;">' +
    lottery_moneyOld +
    '</span>USD</div></div>';
  html +=
    '<div class="clearfix"><div class="pull-left" style="width:100px;">Cá cược:</div><div style="margin-left:100px; text-align: left;white-space:normal;word-break:break-all;" id="ordertotalprice">' +
    lottery_number +
    '</div></div>';

  $('#modal-cacuoc .modal-text').html(html);
  showModal();
  // $.modal({
  //   text: html,
  //   buttons: [
...
function lottery_yuji_money(rate) {
  ...
  for (var i = 0; i < orderList.length; i++) {
    moneynhan +=
      parseFloat(orderList[i].price) * parseFloat(orderList[i].peilv);
  }
  $('.lottery_input_text').html(
    'Tối đa<em style="color:#f4c829;" class="max_money">' +
      parseFloat(moneynhan) +
      '</em>USD',
  );
...
function lottery_touzhufn(number, zhushu) {
  ...
  if (lottery_sum_old_list.length > 0) {
    ...
    if (_this_sum == number) {
      $('#lottery_sum_old_b')
        .find('.lottery_sum_old[lottery_sum="' + _this_sum + '"]')
        .remove();
      if (
        orderList[0].playid == 'k3sbthbz' ||
        orderList[0].playid == 'k3ebthbz'
      ) {
      ...
      betting_sum_moery = isNaN(parseInt($('.lottery_input').val()))
        ? 0
        : parseInt($('.lottery_input').val());
      $('.betting_sum_moery').text(betting_sum_moery * betting_sum);
      ...
      $('#lottery_sum_old_b').append(
        '<em class="lottery_sum_old" lottery_sum="' +
          number +
          '"  >' +
          number +
          '</em>',
...
function showModal() {
  document.getElementById('modal-cacuoc').style.display = 'block';
}
...
var lotterytimes = function (lotteryname) {
  const gameid = $('html').attr('data-dpr');

  clearTimeout(lotterytimesId);
  var ret = null;

  var url = '/api/webapi/k3/GetNoaverageEmerdList';
  $.post(
    url,
    {
      gameJoin: gameid,
    ...
    function (data) {
      ...
      if (data.sign == true) {
        ...
        const timeCreatedTemp = parseInt(lottery?.time) / 1000;
        const timeNowTemp = new Date().getTime() / 1000;
        const time = timeCreatedTemp + timeGame * 2 - timeNowTemp;
        let shortname = '';
        switch (gameId) {
          case '1':
            shortname = '1 phút nhanh';
            break;
          case '3':
            shortname = '3 phút nhanh';
            break;
          case '5':
            shortname = '5 phút nhanh';
            break;
          case '10':
            shortname = 'Venezie gấp 3 lần';
            break;
          case '201':
            shortname = 'Venezie 3 điểm';
            break;
          case '202':
            shortname = 'Venice gần 3';
            break;
          case '203':
            shortname = 'Venice Hà Bắc 3';
            break;
          case '204':
            shortname = 'Venice Thượng Hải 3';
            break;
          case '205':
            shortname = 'Venice Cát Lâm';
            break;
          case '206':
            shortname = 'Venice Bắc 3';
            break;
          case '207':
            shortname = 'Venice Bắc Kinh';
            break;
          default:
            timeGame = 1200;
            break;
        ...
        lottery = {
          shortname: shortname,
          lastFullExpect: lottery?.period,
          currFullExpect: parseInt(lottery?.period) + 1,
          currExpect: parseInt(lottery?.period) + 1,
          lastExpect: lottery?.period,
          openRemainTime: 60,
          remainTime: parseInt(time),
          // remainTime: 300,
        };
        ...
        if (lottery.remainTime && eval(lottery.remainTime) > 1) {
          ...
          if (yIndex > 1) {
            layer.open({
              time: 2,
              content:
                lottery.lastFullExpect + 'Thời gian đặt cược đã kết thúc',
            });
          }

          yIndex++;
          countdownTime(lottery.remainTime, lotterytimes, lotteryname);
          RenderResult(data.data?.gameslist?.[0]?.result);
          ...
          if (ret) {
            ...
            $("[way-data='showExpect.currFullExpect']").text(
              lottery.currFullExpect,
            );
...
var lotteryopencodes = function (lotteryname) {
  ...
  $.post(
    ...
    function (data) {
      data.sign = true;
      if (data.sign == true) {
        var lotlist = data.data?.gameslist;

        for (var o in lotlist) {
          var openinfo = lotlist[o];
          html +=
            '<li class="flex"><p class="hository-orderNum">' +
            openinfo.period +
            '</p>';
          let myFunc = num => Number(num);

          var intArr = Array.from(String(openinfo.result), myFunc);
          var array = intArr;
          var sum =
            parseInt(array[0]) + parseInt(array[1]) + parseInt(array[2]);
          html += '<p class="hository-openNum-k3 flex">';
          for (var ii in array) {
            var num = array[ii];
            html += '<img src="/images/' + num + '.png" />';
          }
          html += '</p>';
          html += '<p class="hository-k3lm font-red">' + sum + '</p>';
          if (sum > 10) {
            html +=
              '<p class="hository-k3lm"><span class="openInfo-ball bgColor-orange">Lớn</span></p>';
          } else {
            html +=
              '<p class="hository-k3lm"><span class="openInfo-ball bgColor-gray">Nhỏ</span></p>';
          }
          if (sum % 2) {
            html +=
              '<p class="hository-k3lm"><span class="openInfo-ball bgColor-orange">Một</span></p>';
          } else {
            html +=
              '<p class="hository-k3lm"><span class="openInfo-ball bgColor-gray">đôi</span></p>';
          }
        }
        $node.html(html);
      } else if (data.sign == false) {
...
var lotteryrates = function () {
  ...
  $.each(rates, function (n, value) {
    ...
    $('.ball_aid[rate_' + _playid + ']').text('tỷ lệ cược' + value.rate);
    //}
  });
...
var loadopencode = function (lotteryname) {
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      msg.sign = true;
      if (msg.sign == true) {
        if (msg.data.gameslist.length > 0) {
          loadopencodecount = 0;
          if (openCodeTimeOut) {
            clearTimeout(openCodeTimeOut);
          }
          ...
          if (lastExpect == openexpect) {
            // 页面开奖号列表赋值
            var openCode = msg.data.gameslist?.[0]?.result;
            let myFunc = num => Number(num);

            var intArr = Array.from(String(openCode), myFunc);
            ...
            var openCodeListVal = $('body').find('.cz_body').html();
            ...
            if (openCodeListVal.indexOf(openexpect) < 0) {
              ...
              if (sum % 2) {
                html +=
                  '<p class="hository-k3lm"><span class="openInfo-ball bgColor-orange">đôi</span></p>';
              } else {
                html +=
                  '<p class="hository-k3lm"><span class="openInfo-ball bgColor-gray">đơn</span></p>';
              }
    ...
    success: function (data) {
      ...
      $.each(data, function (index, val) {
        ...
        if (val.isdraw == -1) {
          // 未中奖绿色
          html +=
            '<div class="item-after"><t style="color:green">未中奖</t></div>';
        } else if (val.isdraw == 1) {
          // 已中奖红色
          html +=
            '<div class="item-after"><t style="color:red">已中奖</t></div>';
        } else if (val.isdraw == -2) {
          html += '<div class="item-after"><del>已撤单</del></div>';
        } else if (val.isdraw == 0) {
          html +=
            '<div class="item-after"><t class="state" onclick="javascript:getBillInfo(\'' +
            val.trano +
            '\')" style="color:red;cursor:pointer;">撤单</t></div>';
        } else {
...
function countdownTime(leftSec, callback, shortName = '5 phút nhanh') {
  var h, m, s, t;
  var h1, m1, s1;
  var h2, m2, s2;
  if (CDTime) {
...
function openLottery1(maxnum) {
  way.set('showExpect.openCode1', Math.round(Math.random() * (maxnum - 1) + 1));
}

function openLottery2(maxnum) {
  way.set('showExpect.openCode2', Math.round(Math.random() * (maxnum - 1) + 1));
}

function openLottery3(maxnum) {
  way.set('showExpect.openCode3', Math.round(Math.random() * (maxnum - 1) + 1));
}

function openLottery4(maxnum) {
  way.set('showExpect.openCode4', Math.round(Math.random() * (maxnum - 1) + 1));
}

function openLottery5(maxnum) {
  way.set('showExpect.openCode5', Math.round(Math.random() * (maxnum - 1) + 1));
}

function openLottery6(maxnum) {
  way.set('showExpect.openCode6', Math.round(Math.random() * (maxnum - 1) + 1));
}

function openLottery7(maxnum) {
  way.set('showExpect.openCode7', Math.round(Math.random() * (maxnum - 1) + 1));
}

function openLottery8(maxnum) {
  way.set('showExpect.openCode8', Math.round(Math.random() * (maxnum - 1) + 1));
}
...
function openLottery10(maxnum) {
  way.set(
    'showExpect.openCode10',
    Math.round(Math.random() * (maxnum - 1) + 1),
  );
}
// 停止开奖
...
function setLotterNumber(_index, _nums) {
  ...
  openLotteryListC = openLotteryListC.substring(
    0,
    openLotteryListC.lastIndexOf('_') + 1,
  );
...
function setslidedd(data) {
  $('.ball_list_ul')
    ...
    .each(function (i) {
      ...
      $(this)
        .find('.peilv')
        .text('赔率' + (dufrate - fandl).toFixed(2));
      lotteryRate = 404;
      lotteryFandian = fans.toFixed(1);
    });
}
...
let lottery_sum_old_bbbb = document.getElementById('lottery_sum_old_b');

function Order_chedan(id, trano, obj) {
  artDialog({
    content: 'Bạn có chắc chắn muốn hủy đơn đặt hàng?',
    cancel: function () {},
    ok: function () {
      $.post(
        '/Apijiekou.chedan',
        ...
        function (json) {
          if (json.sign == true) {
            ...
            $(obj).html(
              '<th style="color:grey;text-align: center;width: 20%;">Đã hủy</th>',
            );
          } else {
            alt(json.message);
          }
...
$('#tz_jl').click(function () {
  ...
  $.ajax({
    ...
    dataType: 'json',
    data: { gameJoin: $('html').attr('data-dpr'), pageno: 0, pageto: 10 },
    cache: false,
    type: 'POST',
    // beforeSend: function () {
    //   $("#jiaz").html("Đang tải....");
    ...
    success: function (html) {
      let htmlRender = '';
      const result = html.data.gameslist;
      result?.map(item => {
        htmlRender += ` <table border="0" style="width:100%">
        <tbody><tr style="border-bottom:1px dotted #383838;font-size:13px;">
            <th style="text-align: center;width: 30%;font-size:0.8em;"><a href="javascript:getBillInfo('H2307072327405')">${
              item.stage
            }</a></th>
            <th style="color:rgb(153, 153, 153);text-align: center;width: 30%;font-size:0.95em;">${
              item.money
            }</th>
            <th style="color:rgb(153, 153, 153);text-align: center;width: 20%;font-size:0.95em;">${
              item.get
            }</th>
            ${
              item?.status === 2
                ? `<th style="text-align: center;width: 20%;color:green">
            Thua
            </th>`
                : `<th style="text-align: center;width: 20%;color:red">
            Thắng
            </th>`
            }
        </tr>
