...
const dailyPageWingo3 = async(req, res) => {
    let auth = req.cookies.auth;
    const [userDL] = await connection.query('SELECT `id_user` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    var idUser = ''+userDL[0].id_user;
    return res.render("daily/a-index-bet/index3.ejs", { idUser }); 
}

const dailyPageWingo5 = async(req, res) => {
    let auth = req.cookies.auth;
    const [userDL] = await connection.query('SELECT `id_user` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    var idUser = ''+userDL[0].id_user;
    return res.render("daily/a-index-bet/index5.ejs", { idUser }); 
}

const dailyPageWingo10 = async(req, res) => {
    let auth = req.cookies.auth;
    const [userDL] = await connection.query('SELECT `id_user` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    var idUser = ''+userDL[0].id_user;
    return res.render("daily/a-index-bet/index10.ejs", { idUser }); 
}

const dailyPageK3 = async(req, res) => {
    let auth = req.cookies.auth;
    const [userDL] = await connection.query('SELECT `id_user` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    var idUser = ''+userDL[0].id_user;
    return res.render("daily/k3.ejs", { idUser }); 
}
const dailyPageK5 = async(req, res) => {
    let auth = req.cookies.auth;
    const [userDL] = await connection.query('SELECT `id_user` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    var idUser = ''+userDL[0].id_user;
    return res.render("daily/k5.ejs", { idUser }); 
}
const dailyPageWingo = async(req, res) => {
    let auth = req.cookies.auth;
    const [userDL] = await connection.query('SELECT `id_user` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    var idUser = ''+userDL[0].id_user;
    return res.render("daily/wingo.ejs", { idUser }); 
}
...
const createBonus = async(req, res) => {
    ...
    if (ctv.money - money >= 0) {
        ...
        return res.status(200).json({
            message: 'Số dư không đủ để tạo quà',
            status: false,
        });
    }
}
