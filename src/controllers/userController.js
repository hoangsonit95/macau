...
const changePassword = async (req, res) => {
  ...
  if (newPassWord.length > 12)
    return res.status(200).json({
      message: "Độ dài mật khẩu từ 3 đến 12 ký tự",
      status: false,
      timeStamp: timeNow,
    });
  const [rows] = await connection.query(
  ...
  if (data) {
    if (data == 1) {
      ...
      if (check >= data && point_list.total1 != 0) {
        ...
        return res.status(200).json({
          message: `Bạn vừa nhận được ${point_list.total1}.00USD`,
          status: true,
          timeStamp: timeNow,
        });
    ...
    if (data == 2) {
      ...
      if (check >= get && point_list.total2 != 0) {
        ...
        return res.status(200).json({
          message: `Bạn vừa nhận được ${point_list.total2}.00USD`,
          status: true,
          timeStamp: timeNow,
        });
    ...
    if (data == 3) {
      ...
      if (check >= get && point_list.total3 != 0) {
        ...
        return res.status(200).json({
          message: `Bạn vừa nhận được ${point_list.total3}.00USD`,
          status: true,
          timeStamp: timeNow,
        });
    ...
    if (data == 4) {
      ...
      if (check >= get && point_list.total4 != 0) {
        ...
        return res.status(200).json({
          message: `Bạn vừa nhận được ${point_list.total4}.00USD`,
          status: true,
          timeStamp: timeNow,
        });
    ...
    if (data == 5) {
      ...
      if (check >= get && point_list.total5 != 0) {
        ...
        return res.status(200).json({
          message: `Bạn vừa nhận được ${point_list.total5}.00USD`,
          status: true,
          timeStamp: timeNow,
        });
    ...
    if (data == 6) {
      ...
      if (check >= get && point_list.total6 != 0) {
        ...
        return res.status(200).json({
          message: `Bạn vừa nhận được ${point_list.total6}.00USD`,
          status: true,
          timeStamp: timeNow,
        });
    ...
    if (data == 7) {
      ...
      if (check >= get && point_list.total7 != 0) {
        ...
        return res.status(200).json({
          message: `Bạn vừa nhận được ${point_list.total7}.00USD`,
          status: true,
          timeStamp: timeNow,
        });
...
const infoUserBank = async (req, res) => {
  ...
  const [recharge] = await connection.query(
    "SELECT * FROM recharge WHERE phone = ? AND statusTongCuoc = 0 AND status = 1 ",
    [userInfo.phone],
  );
  ...
  if (userBank.length > 0) {
    userBank[0].stk = userBank[0].stk.slice(0, 3) + "*****";
  }
...
const withdrawal3 = async (req, res) => {
  ...
  let money = req.body.money;
  let password = req.body.password;
  let passwordWithdraw = req.body.passwordWithdraw;
  if (!auth || !money || !password || !passwordWithdraw) {
    return res.status(200).json({
      message: "Failed",
      status: false,
      timeStamp: timeNow,
  ...
  const [user] = await connection.query(
    "SELECT `phone`, `code`,`invite`, `money`, `tongcuoc` FROM users WHERE token = ? AND password = ? AND password_withdraw = ?",
    [auth, md5(password), md5(passwordWithdraw)],
  );
  if (user.length == 0) {
    return res.status(200).json({
      message: "Mật khẩu đăng nhập hoặc mật khẩu rút tiền không chính xác",
      status: false,
      timeStamp: timeNow,
    });
  }
  ...
  if (userInfo.money < 0) {
    return res.status(200).json({
      message: "Số tiền không đúng quy định",
      status: false,
      timeStamp: timeNow,
    });
  }
  ...
  let tongcuoc = parseInt(userInfo.tongcuoc);
  // minutes_1.forEach((data) => {
  //     total2 += data.money;
  // });

  let result = userInfo.tongcuoc == 0 ? true : false;
  ...
  if (user_bank.length != 0) {
    if (withdraw.length < 3) {
      ...
      return res.status(200).json({
        message: "Mỗi ngày bạn chỉ được tối đa 3 lần rút tiền",
        status: false,
        timeStamp: timeNow,
      });
    }
...
const LaunchGame = async (req, res, next) => {
  try {
    ...
    let data = req.body;
    data.token = token;
    let result = await Users.LaunchGame(data);
...
const listApi = async (req, res, next) => {
  try {
    let token = req.cookies.auth;
    let result = await Users.getBalance(token);
    return res.status(200).json(result);
  } catch (error) {
    next(error);
  }
...
const tranfer = async (req, res, next) => {
  try {
    ...
    let data = req.body;
    data.token = token;
    let result = await Users.tranfer(data);
    return res.status(200).json(result);
  } catch (error) {
    next(error);
  }
