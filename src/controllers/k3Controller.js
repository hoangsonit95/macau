...
const K3Page = async (req, res) => {
  return res.render('bet/k3/k3.ejs', { title: 'Bông Sen' });
};
  ...
  try {
    ...
    if (check >= 0) {
      ...
      const [waiting] = await connection.query(
        `SELECT id, phone, money, price, amount, bet, game, typeGame FROM result_k3 WHERE status = ? and phone = ? ORDER BY id DESC LIMIT 1`,
        [0, userInfo.phone],
      );
      return res.status(200).json({
...
const addK3 = async game => {
  try {
    ...
    const foundSetting = setting.find(setting => setting.hasOwnProperty(join));
    let nextResult = foundSetting[join];
...
async function plusMoney(game) {
  ...
  // let join = `k3d${game == 1 ? '' : game}`;
  // const [priceGet] = await connection.execute(
  //   `SELECT * FROM price_k3 WHERE game = ? `,
  //   [join],
  // );
  const [priceGet] = await connection.execute(
    `SELECT * FROM price_k3 WHERE game = 'k3d' `
  );
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    if (orders.typeGame == 'total') {
      ...
      if (totalResult2 % 2 == 0 && lengWin2.includes('c')) {
        const value_position = priceGet.find(
          item => item['game_code'] === 1 && item.position == 'c',
        );
        const value = Number(value_position?.value);
        nhan_duoc += price * value * orders.amount;
      }

      if (totalResult2 % 2 != 0 && lengWin2.includes('l')) {
        const value_position = priceGet.find(
          item => item['game_code'] === 1 && item.position == 'l',
        );
        const value = Number(value_position?.value);
        nhan_duoc += price * value * orders.amount;
      }
      ...
      if (totalResult2 >= 11 && totalResult2 <= 18 && lengWin2.includes('b')) {
        // nhan_duoc += price * 1.92 * orders.amount;
        const value_position = priceGet.find(
          item => item['game_code'] === 1 && item.position == 'b',
        );
        const value = Number(value_position?.value);
        nhan_duoc += price * value * orders.amount;
      }
      ...
      if (totalResult2 >= 3 && totalResult2 <= 11 && lengWin2.includes('s')) {
        // nhan_duoc += price * 1.92 * orders.amount;
        const value_position = priceGet.find(
          item => item['game_code'] === 1 && item.position == 's',
        );
        const value = Number(value_position?.value);
        nhan_duoc += price * value * orders.amount;
      }
      ...
      switch (lengWin[0]) {
        case '3':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '3',
          );
          get = Number(value_position?.value);
          break;
        case '4':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '4',
          );
          get = Number(value_position?.value);
          break;
        case '5':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '5',
          );
          get = Number(value_position?.value);
          break;
        case '6':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '6',
          );
          get = Number(value_position?.value);
          break;
        case '7':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '7',
          );
          get = Number(value_position?.value);
          break;
        case '8':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '8',
          );
          get = Number(value_position?.value);
          break;
        case '9':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '9',
          );
          get = Number(value_position?.value);
          break;
        case '10':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '10',
          );
          get = Number(value_position?.value);
          break;
        case '11':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '11',
          );
          get = Number(value_position?.value);
          break;
        case '12':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '12',
          );
          get = Number(value_position?.value);
          break;
        case '13':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '13',
          );
          get = Number(value_position?.value);
          break;
        case '14':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '14',
          );
          get = Number(value_position?.value);
          break;
        case '15':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '15',
          );
          get = Number(value_position?.value);
          break;
        case '16':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '16',
          );
          get = Number(value_position?.value);
          break;
        case '17':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '17',
          );
          get = Number(value_position?.value);
          break;
        case '18':
          value_position = priceGet.find(
            item => item['game_code'] === 1 && item.position == '18',
          );
          get = Number(value_position?.value);
          break;
      }
      nhan_duoc += price * get * orders.amount;
      await connection.execute(
        'UPDATE `result_k3` SET `get` = ?, `status` = 1 WHERE `id` = ? AND game = ? ',
        [nhan_duoc, id, game],
      );
      ...
      for (let i = 0; i < arr1.length; i++) {
        if (arr1[i] != '') {
          ...
          if (check1 || check2) {
            ...
            const value_position_game2 = priceGet.find(
              item => item['game_code'] === 2 && item.position == '1',
            );
            const value = Number(value_position_game2?.value);
            nhan_duoc += total * value;
          }
        }
      }
      arr2 = array[1];
      ...
      for (let i = 0; i < arr1.length; i++) {
        if (arr1[i] != '') {
          ...
          if (check1) {
            ...
            const value_position_game3 = priceGet.find(
              item => item['game_code'] === 3 && item.position == 'diff',
            );
            const value = Number(value_position_game3?.value);

            nhan_duoc += total * value - orders.fee;
          }
        }
      }
      if (arr2 != '') {
        ...
        const value_position_game3 = priceGet.find(
          item => item['game_code'] === 3 && item.position == 'same',
        );
        const value = Number(value_position_game3?.value);
      ...
      for (let i = 0; i < arr1.length; i++) {
        if (arr1[i] != '') {
          ...
          if (!check1) {
            ...
            const value_position_game4 = priceGet.find(
              item => item['game_code'] === 4 && item.position == '3',
            );
            const value = Number(value_position_game4?.value);
            ...
            if (arr2 == 'u') {
              ...
              const value_position_game4 = priceGet.find(
                item => item['game_code'] === 4 && item.position == 'consec',
              );
              const value = Number(value_position_game4?.value);
              nhan_duoc += (total - orders.fee) * value;
            }
          }
        }
      }
      ...
      for (let i = 0; i < arr3.length; i++) {
        if (arr1[i] != '') {
          ...
          if (!check1) {
            ...
            const value_position_game4 = priceGet.find(
              item => item['game_code'] === 4 && item.position == '2',
            );
            const value = Number(value_position_game4?.value);
            nhan_duoc += total * value - orders.fee;
          }
