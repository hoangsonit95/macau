  ...
  try {
    ...
    if (rows.length == 1) {
      if (rows[0].status == 1) {
        ...
        await User.createPlayer(md5(accessToken));
        return res.status(200).json({
...
const register = async (req, res) => {
  ...
  let { username, pwd, invitecode, passwordWithdraw } = req.body;
  ...
  if (!username || !pwd || !invitecode || !passwordWithdraw) {
    return res.status(200).json({
      message: "ERROR!!!",
      status: false,
    });
  ...
  if (username.length != 9 || !isNumber(username)) {
    return res.status(200).json({
      message: "SĐT không đúng, vui lòng bỏ số không và nhập 9 số còn lại",
      status: false,
    });
  }
    ...
    if (check_u.length == 1 && check_u[0].veri == 1) {
      ...
      if (check_i.length == 1) {
        if (check_ip.length <= 3) {
          ...
          const sql =
            "INSERT INTO users SET id_user = ?,phone = ?,name_user = ?,password = ?,password_withdraw = ?,money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?";
          await connection.execute(sql, [
            id_user,
            username,
            name_user,
            md5(pwd),
            md5(passwordWithdraw),
            0,
            code,
            invitecode,
            ctv,
          ...
          await User.save({ username, password: pwd });
          return res.status(200).json({
